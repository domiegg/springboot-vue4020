	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="快递类别" prop="kuaidileibie">
					<el-select :disabled="ro.kuaidileibie" v-model="ruleForm.kuaidileibie" placeholder="请选择快递类别" >
						<el-option
							v-for="(item,index) in kuaidileibieOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="快递类别" prop="kuaidileibie">
					<el-input v-model="ruleForm.kuaidileibie"
						placeholder="快递类别" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="报价" prop="baojia">
					<el-input-number v-model="ruleForm.baojia" placeholder="报价" :disabled="ro.baojia"></el-input-number>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'" label="收件人账号" prop="zhanghao">
					<el-select :disabled="ro.zhanghao" @change="zhanghaoChange" v-model="ruleForm.zhanghao" placeholder="请选择收件人账号">
						<el-option
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.zhanghao" label="收件人账号" prop="zhanghao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="寄件时间" prop="jijianshijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.jijianshijian" 
						type="datetime"
						:readonly="ro.jijianshijian"
						placeholder="寄件时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.jijianshijian" label="寄件时间" prop="jijianshijian">
					<el-input v-model="ruleForm.jijianshijian" placeholder="寄件时间" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	...
	created() {
		this.ruleForm.jijianshijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jijianshijian'){
							this.ruleForm.jijianshijian = obj[o];
							this.ro.jijianshijian = true;
							continue;
						}
						if(o=='crossuserid'){
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.zhanghao!=''&&json.zhanghao) || json.zhanghao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.jijianzhanghao = json.zhanghao
						this.ro.jijianzhanghao = true;
					}
					if(((json.xingming!=''&&json.xingming) || json.xingming==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.jijianxingming = json.xingming
						this.ro.jijianxingming = true;
					}
					if(((json.shouji!=''&&json.shouji) || json.shouji==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.jijianshouji = json.shouji
						this.ro.jijianshouji = true;
					}
				} else {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `kuaidiluru/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.kuaidiluruCrossAddOrUpdateFlag = false;
											this.parent.search();
