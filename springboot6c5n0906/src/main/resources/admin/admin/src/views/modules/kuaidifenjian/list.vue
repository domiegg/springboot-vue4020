<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('kuaidifenjian','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('kuaidifenjian','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="分拣时间">
						<template slot-scope="scope">
							{{scope.row.fenjianshijian}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kuaidifenjian','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('kuaidifenjian','快递运输')" @click="kuaidiyunshuCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','请勿重复操作!')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								快递运输
							</el-button>
							<el-button class="edit" v-if=" isAuth('kuaidifenjian','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kuaidifenjian','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<kuaidiyunshu-cross-add-or-update v-if="kuaidiyunshuCrossAddOrUpdateFlag" :parent="this" ref="kuaidiyunshuCrossaddOrUpdate"></kuaidiyunshu-cross-add-or-update>
...
import kuaidiyunshuCrossAddOrUpdate from "../kuaidiyunshu/add-or-update";
	export default {
		data() {
			return {
				diquOptions: [],
				...
				kuaidiyunshuCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			kuaidiyunshuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kuaidiyunshuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','kuaidifenjian');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kuaidiyunshuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kuaidiyunshuCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	kuaidifenjianstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kuaidifenjian/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
