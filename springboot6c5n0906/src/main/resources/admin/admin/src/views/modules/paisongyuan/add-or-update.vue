<template>
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="派送员账号" prop="paisongyuanzhanghao">
					<el-input v-model="ruleForm.paisongyuanzhanghao" placeholder="派送员账号" clearable  :readonly="ro.paisongyuanzhanghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="派送员账号" prop="paisongyuanzhanghao">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				paisongyuanzhanghao: [
					{ required: true, message: '派送员账号不能为空', trigger: 'blur' },
				],
				paisongyuanxingming: [
					{ required: true, message: '派送员姓名不能为空', trigger: 'blur' },
				],
				xingbie: [
				],
				shouji: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `paisongyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.paisongyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
