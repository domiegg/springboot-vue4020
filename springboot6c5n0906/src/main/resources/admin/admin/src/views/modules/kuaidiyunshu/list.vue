<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"left","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="运输状态" prop="yunshuzhuangtai">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">运输状态</label>
						<el-select clearable v-model="searchForm.yunshuzhuangtai" placeholder="请选择运输状态" >
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('kuaidiyunshu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('kuaidiyunshu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","padding":"0","margin":"10px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('kuaidiyunshu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('kuaidiyunshu','快递配送')" @click="kuaidipeisongCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','请勿重复操作!')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								快递配送
							</el-button>
							<el-button class="edit" v-if=" isAuth('kuaidiyunshu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kuaidiyunshu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<kuaidipeisong-cross-add-or-update v-if="kuaidipeisongCrossAddOrUpdateFlag" :parent="this" ref="kuaidipeisongCrossaddOrUpdate"></kuaidipeisong-cross-add-or-update>
...
import kuaidipeisongCrossAddOrUpdate from "../kuaidipeisong/add-or-update";
	export default {
		data() {
			return {
				...
				kuaidipeisongCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			kuaidipeisongCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kuaidipeisongCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','kuaidiyunshu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kuaidipeisongCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kuaidipeisongCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	kuaidiyunshustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'kuaidiyunshu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
