	...
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="分拣时间" prop="fenjianshijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.fenjianshijian" 
						type="datetime"
						:readonly="ro.fenjianshijian"
						placeholder="分拣时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.fenjianshijian" label="分拣时间" prop="fenjianshijian">
					<el-input v-model="ruleForm.fenjianshijian" placeholder="分拣时间" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	...
	created() {
		this.ruleForm.fenjianshijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fenjianshijian'){
							this.ruleForm.fenjianshijian = obj[o];
							this.ro.fenjianshijian = true;
							continue;
						}
				}
			}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `kuaidifenjian/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.kuaidifenjianCrossAddOrUpdateFlag = false;
											this.parent.search();
